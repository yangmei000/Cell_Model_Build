...
data_read = pd.read_excel(r"细胞建模第二次数据  2023.9.24.xlsx")
# print(data_read.columns)
data_read.drop(["序号",'ID号'], axis=1, inplace=True)
...
data_read['结局_症状持续时间；1≤7天；2>7'] = data_read['结局_症状持续时间；1≤7天；2>7'].astype(
    'category')
data_read['结局_症状持续时间；1≤7天；2>7'] = data_read['结局_症状持续时间；1≤7天；2>7'].apply(
    lambda x: 0 if x == 1 else x)
data_read['结局_症状持续时间；1≤7天；2>7'] = data_read['结局_症状持续时间；1≤7天；2>7'].apply(
    lambda x: 1 if x == 2 else x)
y = data_read.get('结局_症状持续时间；1≤7天；2>7')
X = data_read.drop(['结局_症状持续时间；1≤7天；2>7'], axis=1)
